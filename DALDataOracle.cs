...
namespace DataAccess
{
    public class DALDataOracle: DALDataBase
    {
               ...
               dynamic cmdConnectDynamic = null, dynamic cmdTransactDynamic = null)
        {
            ...
            try
            {
                ...
                if (cmdConnectDynamic != null) cmdConnect = (OracleConnection)cmdConnectDynamic;

                //if (cmdTransactDynamic != null) cmdTransact = (OleDbTransaction)cmdTransactDynamic;
                if (cmdTransactDynamic != null) cmdTransact = (OracleTransaction)cmdTransactDynamic;
        ...
        public override Int64 GetScalar(string cmdText, Dictionary<string, dynamic> cmdParams = null)
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand(cmdText, cmdConnect);
                cmd.CommandType = CommandType.Text;

                // add parameters to command object
                //ParamValue(cmdParam.cmdParams, key)
                if (cmdParams != null)
        ...
        {
            OracleCommand cmd = (OracleCommand)GetDataReaderCommand(cmdParam);
        ...
        {
            OracleConnection cmdConnect = new OracleConnection(connectionString);
            if (cmdConnect.State != ConnectionState.Open) cmdConnect.Open();
        ...
        {
            OracleDataReader reader = (OracleDataReader)rdr;
        ...
        public void BuildNodeLocation(int parentId, string parentCode = "", OracleConnection cn = null, string chars = "")
        {
            ...
            OracleConnection cnObj;
            OracleCommand cmd;
            OracleDataReader rdr;

            if (cn == null)
            {
                cnObj = new OracleConnection(connectionString);
                cnObj.Open();

                cmd = new OracleCommand("select lchars from sys_LocationCharacters order by chr_order", cnObj);
                cmd.CommandType = CommandType.Text;
            ...
            if (cn == null)
            {
                // cmd = new OracleCommand(String.Format("update {0} set tre_nod_loc=" + DALData.PARAM_PREFIX + "p1 where tre_nod_tag=" + DALData.PARAM_PREFIX + "p2;", treTableName), cnObj);

                cmd = new OracleCommand(String.Format("update {0} set tre_nod_loc=" + DALData.PARAM_PREFIX + "p1 where tre_nod_tag=" + DALData.PARAM_PREFIX + "p2", treTableName), cnObj);
                cmd.Parameters.Add(new OracleParameter(DALData.PARAM_PREFIX + "p1", parentCode));
                cmd.Parameters.Add(new OracleParameter(DALData.PARAM_PREFIX + "p2", parentId));
                cmd.CommandType = CommandType.Text;
                cmd.ExecuteNonQuery();
                cmd.Dispose();
            }

            cmd = new OracleCommand(String.Format("select tre_nod_tag from {0} where tre_nod_tag_par=" + DALData.PARAM_PREFIX + "p1", treTableName), cnObj);
            cmd.CommandType = CommandType.Text;
            cmd.Parameters.Clear();
            cmd.Parameters.Add(new OracleParameter(DALData.PARAM_PREFIX + "p1", parentId));
            rdr = cmd.ExecuteReader();
            ...
            while (rdr.Read())
            {
                ...
                OracleCommand ucmd = new OracleCommand(String.Format("update {0} set tre_nod_loc=" + DALData.PARAM_PREFIX + "p1 where tre_nod_tag=" + DALData.PARAM_PREFIX + "p2", treTableName), cnObj);
                ucmd.Parameters.Clear();
                ucmd.Parameters.Add(new OracleParameter(DALData.PARAM_PREFIX + "p1", chCode));
                ucmd.Parameters.Add(new OracleParameter(DALData.PARAM_PREFIX + "p2", cid));
    ...
    {
        public DALOracleConnection(bool beginTransaction = false)
        {
            // Creates a new connection with Transaction initiated
            try
            {
                // Create new connection
                this.Connection = new OracleConnection(DALData.DAL.connectionString);
